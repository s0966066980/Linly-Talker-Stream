    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in tqdm(enumerate(frames)):
            ...
            if preload:
                ...
                bg = torso_img[..., :3] * torso_img[..., 3:] / 255.0 + current_bg_img * (1 - torso_img[..., 3:] / 255.0)
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".jpg", args=None):
    ...
    train_cam_infos = readCamerasFromTransforms(path, "transforms_train.json", white_background, extension, audio_file, audio_extractor, eval= eval, width= args.image_width, height= args.image_height, bg_img_config=bg_img_config)
    print("Reading Test Transforms")
    test_cam_infos = readCamerasFromTransforms(path, "transforms_val.json", white_background, extension, audio_file, audio_extractor, eval = eval, width= args.image_width, height= args.image_height, bg_img_config=bg_img_config)
